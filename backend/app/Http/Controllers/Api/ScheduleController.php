...
class ScheduleController extends Controller
{
    ...
    public function storeEvent(Request $request)
    {
        ...
        // Tìm checkin_place đầu tiên hoặc tạo một checkin_place mặc định
        $checkinPlace = \App\Models\CheckInPlace::first();
        
        if (!$checkinPlace) {
            return response()->json([
                'message' => 'Không có địa điểm check-in nào trong hệ thống',
                'error' => 'no_checkin_places'
            ], 400);
        }
        ...
        $schedule = Schedule::create([
            ...
            'participants' => 1,
            'budget' => 0,
            'checkin_place_id' => $checkinPlace->id, // Sử dụng checkin_place đầu tiên
        ]);
    ...
    public function getUserEvents()
    {
        ...
        $events = Schedule::where('user_id', auth()->id())
            ->select('id', 'name as title', 'start_date', 'end_date', 'description', 'status', 'user_id')
            ->with('user:id,name') // Thêm relationship với user
            ->get()
            ->map(function($event) {
                ...
                // Nếu chỉ có date (không có time), thì đó là all-day event
                if (strlen($startDate) <= 10) {
                    $event->start = $startDate;
                    $event->end = $endDate;
                } else {
                    // Nếu có time, convert thành ISO format
