...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('checkin_places', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('location_id')->nullable(); // Khóa ngoại đến bảng locations (có thể null)
            $table->decimal('price', 15, 2)->nullable(); // Giá vé (có thể null)
            $table->boolean('is_free')->default(false); // Có miễn phí hay không (mặc định là có phí)
            ...
            $table->text('caption')->nullable(); // Chú thích mô tả địa điểm (có thể null)
            $table->integer('distance')->nullable(); // Khoảng cách (phút hoặc km, có thể null)
            $table->json('transport_options')->nullable(); // Phương tiện di chuyển (JSON, có thể null)
            $table->timestamps(); // created_at và updated_at

            $table->foreign('location_id')
                ->references('id')
                ->on('locations')
                ->onDelete('set null'); // Nếu xóa location thì set null
        });
    }
