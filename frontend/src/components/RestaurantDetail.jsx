...
const RestaurantDetail = ({ restaurantId, onBack }) => {
  ...
  const formatRating = (rating) => (rating ? Number.parseFloat(rating).toFixed(1) : "0.0")
  ...
  if (error || !restaurant) {
    return (
      <div className="text-center py-12">
        <p className="text-red-500 mb-4">{error || "Không tìm thấy nhà hàng"}</p>
        <button onClick={onBack} className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-lg">
          Quay lại
        </button>
      </div>
    )
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Thông tin nhà hàng</h2>
          ...
          {/* Menu */}
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Thực đơn nổi bật</h2>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Đánh giá khách hàng</h2>
            {reviews.length > 0 ? (
                  ...
                  <div key={index} className="border-b border-gray-200 pb-4 last:border-b-0">
                    <div className="flex items-start space-x-3">
                      <div className="w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center">
                        <span className="text-gray-600 font-medium">{`U${index + 1}`}</span>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          <span className="font-medium text-gray-900">Khách hàng {index + 1}</span>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="text-center mb-4">
              <div className="text-3xl font-bold text-gray-900 mb-1">{formatRating(restaurant.rating)}</div>
              <div className="flex justify-center mb-1">{renderStars(restaurant.rating)}</div>
              <p className="text-gray-600 text-sm">{reviewStats?.total_reviews || 0} đánh giá</p>
            </div>
                  ...
                  <div key={rating} className="flex items-center space-x-2">
                    ...
                    <div className="flex-1 bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${reviewStats.rating_distribution[rating]?.percentage || 0}%`,
                        }}
                      ></div>
                    </div>
