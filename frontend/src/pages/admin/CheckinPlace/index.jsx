import React, { useEffect, useState, useMemo, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import {
  getAllCheckinPlaces,
  deleteCheckinPlace,
  getCheckinPlaceStatistics,
} from "../../../services/ui/CheckinPlace/checkinPlaceService.js"; // Đã điều chỉnh đường dẫn và thêm .js

const CheckinPlaceList = () => {
  const [places, setPlaces] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 7;
  const [statistics, setStatistics] = useState({
    totalCheckinPlaces: 0,
    totalReviews: 0,
    totalCheckins: 0,
    activeCheckinPlaces: 0,
  });
  const [isSelectionMode, setIsSelectionMode] = useState(false); // State để kiểm soát chế độ chọn
  const [selectedItems, setSelectedItems] = useState(new Set()); // Set để lưu trữ các ID đã chọn

  const navigate = useNavigate();

  // Hàm tải số liệu thống kê (sử dụng useCallback để tránh tạo lại hàm không cần thiết)
  const fetchStatistics = useCallback(async () => {
    try {
      const res = await getCheckinPlaceStatistics();
      if (res.success) {
        setStatistics(res.data);
      } else {
        console.error("❌ Lỗi khi tải số liệu thống kê:", res.message);
      }
    } catch (err) {
      console.error("❌ Lỗi khi gọi API thống kê:", err);
    }
  }, []);

  // Hàm tải danh sách địa điểm check-in (sử dụng useCallback)
  const loadPlaces = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getAllCheckinPlaces();
      const allPlaces = res.data.data || [];
      setPlaces(allPlaces);
    } catch (err) {
      console.error("❌ Lỗi khi tải danh sách địa điểm:", err);
    } finally {
      setLoading(false);
    }
  }, []);

  // useEffect để tải dữ liệu khi component mount
  useEffect(() => {
    loadPlaces(); // Tải danh sách địa điểm
    fetchStatistics(); // Tải số liệu thống kê
  }, [loadPlaces, fetchStatistics]);

  // Hàm xử lý khi nhấn nút "Chọn xóa" / "Hủy"
  const toggleSelectionMode = () => {
    setIsSelectionMode((prev) => !prev);
    setSelectedItems(new Set()); // Reset các lựa chọn khi chuyển đổi chế độ
  };

  // Hàm xử lý khi chọn/bỏ chọn một mục
  const handleSelectItem = (id) => {
    setSelectedItems((prev) => {
      const newSelection = new Set(prev);
      if (newSelection.has(id)) {
        newSelection.delete(id);
      } else {
        newSelection.add(id);
      }
      return newSelection;
    });
  };

  // Hàm xử lý khi chọn/bỏ chọn tất cả các mục
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allIds = new Set(filteredPlaces.map((place) => place.id)); // Chỉ chọn các mục đã lọc [cite: 17]
      setSelectedItems(allIds);
    } else {
      setSelectedItems(new Set());
    }
  };

  // Hàm xử lý xóa nhiều địa điểm đã chọn
  const handleDeleteSelected = async () => {
    if (selectedItems.size === 0) {
      alert("Vui lòng chọn ít nhất một mục để xóa."); // [cite: 20]
      return;
    }

    if (
      window.confirm(
        `Bạn có chắc muốn xóa ${selectedItems.size} địa điểm đã chọn không? Hành động này không thể hoàn tác!`
      )
    ) {
      try {
        const deletionPromises = Array.from(selectedItems).map((id) =>
          deleteCheckinPlace(id)
        );
        await Promise.all(deletionPromises); // [cite: 21]

        alert(`✅ Đã xóa thành công ${selectedItems.size} địa điểm đã chọn!`);
        setSelectedItems(new Set()); // Xóa lựa chọn [cite: 22]
        setIsSelectionMode(false); // Thoát chế độ chọn [cite: 23]
        loadPlaces(); // Tải lại danh sách sau khi xóa [cite: 24]
        fetchStatistics(); // Cập nhật lại thống kê sau khi xóa [cite: 25]
      } catch (err) {
        console.error("❌ Xóa thất bại:", err); // [cite: 26]
        alert("❌ Xóa thất bại. Vui lòng thử lại sau.");
      }
    }
  };

  // Hàm xử lý xóa địa điểm đơn lẻ
  const handleDelete = async (id) => {
    if (
      window.confirm(
        `Bạn có chắc chắn muốn xóa địa điểm này không? Hành động này không thể hoàn tác!`
      )
    ) {
      try {
        await deleteCheckinPlace(id); // [cite: 27]
        alert(`✅ Đã xóa thành công địa điểm!`); // [cite: 28]
        loadPlaces(); // Tải lại danh sách sau khi xóa
        fetchStatistics(); // Cập nhật lại thống kê sau khi xóa [cite: 29]
      } catch (err) {
        alert("❌ Xóa thất bại. Vui lòng thử lại sau."); // [cite: 29]
        console.error("Lỗi khi xóa địa điểm:", err); // [cite: 30]
      }
    }
  };

  // Hàm render hình ảnh địa điểm hoặc placeholder
  const renderPlaceImage = useCallback((imagePath, altText) => {
    // [cite: 31]
    if (imagePath) {
      return (
        <img
          src={`http://localhost:8000/storage/${imagePath}`}
          alt={altText}
          className="w-10 h-10 rounded-md object-cover mr-3"
          onError={(e) => {
            e.target.onerror = null;
            e.target.src = "https://via.placeholder.com/40x40?text=No"; // [cite: 32]
          }}
        />
      );
    }
    return (
      <div className="w-10 h-10 bg-gray-200 rounded-md flex items-center justify-center text-xs text-gray-500 mr-3">
        No
      </div>
    );
  }, []);

  // Hàm trả về nhãn trạng thái
  const getStatusLabel = useCallback((status) => {
    // [cite: 33]
    switch (status) {
      case "active":
        return "Mở cửa";
      case "inactive":
        return "Đóng cửa";
      case "draft":
        return "Bản nháp";
      default:
        return "Không rõ";
    }
  }, []);

  // Lọc địa điểm dựa trên tìm kiếm (sử dụng useMemo để tối ưu)
  const filteredPlaces = useMemo(() => {
    // [cite: 34]
    return places.filter(
      (place) =>
        place.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        place.address?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (place.id && String(place.id).includes(searchTerm))
    );
  }, [places, searchTerm]);

  // Logic phân trang
  const indexOfLastItem = currentPage * itemsPerPage; // [cite: 35]
  const indexOfFirstItem = indexOfLastItem - itemsPerPage; // [cite: 35]
  const currentItems = filteredPlaces.slice(indexOfFirstItem, indexOfLastItem); // [cite: 36]
  const totalPages = Math.ceil(filteredPlaces.length / itemsPerPage); // [cite: 36]

  const paginate = useCallback(
    // [cite: 37]
    (pageNumber) => {
      if (pageNumber < 1 || pageNumber > totalPages) return;
      setCurrentPage(pageNumber);
    },
    [totalPages]
  );

  const getPaginationNumbers = useCallback(() => {
    // [cite: 38]
    const delta = 2;
    const range = [];
    const rangeWithDots = [];
    let l;

    range.push(1);
    for (let i = currentPage - delta; i <= currentPage + delta; i++) {
      if (i < totalPages && i > 1) {
        range.push(i);
      }
    }
    range.push(totalPages);

    const uniqueRange = [...new Set(range)].sort((a, b) => a - b); // [cite: 39]

    for (let i of uniqueRange) {
      if (l) {
        if (i - l === 2) {
          rangeWithDots.push(l + 1);
        } else if (i - l !== 1) {
          rangeWithDots.push("...");
        }
      }
      rangeWithDots.push(i);
      l = i;
    } // [cite: 40]
    return rangeWithDots;
  }, [currentPage, totalPages]);

  return (
    // [cite: 41]
    <div className="min-h-screen bg-gray-100 font-sans">
      {/* Header and User Profile */}
      <header className="bg-white shadow p-4 flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">
          Quản lý điểm check-in
        </h1>
        <div className="flex items-center space-x-4">
          <div className="relative">
            <img
              src="https://i.pravatar.cc/40?img=1" // [cite: 42]
              alt="User Avatar"
              className="w-10 h-10 rounded-full cursor-pointer"
            />
            <span className="absolute top-0 right-0 block h-2 w-2 rounded-full ring-2 ring-white bg-red-500"></span>
          </div>
          <span className="text-gray-700">Admin</span>
          <img
            src="https://i.pravatar.cc/40?img=2" // [cite: 43]
            alt="Admin Avatar"
            className="w-10 h-10 rounded-full"
          />
        </div>
      </header>

      {/* Statistics Cards */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6">
        <div className="bg-white p-4 rounded-lg shadow-sm flex items-center justify-between">
          {" "}
          {/* [cite: 44] */}
          <div>
            <div className="text-sm text-gray-500">
              Tổng số địa điểm check-in
            </div>
            <div className="text-3xl font-bold text-gray-800">
              {statistics.totalCheckinPlaces.toLocaleString()}{" "}
              {/* [cite: 45] */}
            </div>
          </div>
          <div className="w-12 h-12 flex items-center justify-center rounded-full bg-blue-100 text-blue-600">
            <i className="fas fa-map-marker-alt"></i>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm flex items-center justify-between">
          <div>
            <div className="text-sm text-gray-500">Tổng lượt đánh giá</div>{" "}
            {/* [cite: 46] */}
            <div className="text-3xl font-bold text-gray-800">
              {statistics.totalReviews.toLocaleString()}
            </div>
          </div>
          <div className="w-12 h-12 flex items-center justify-center rounded-full bg-yellow-100 text-yellow-600">
            <i className="fas fa-star"></i>
          </div>
        </div>{" "}
        {/* [cite: 47] */}
        <div className="bg-white p-4 rounded-lg shadow-sm flex items-center justify-between">
          <div>
            <div className="text-sm text-gray-500">Tổng lượt check-in</div>
            <div className="text-3xl font-bold text-gray-800">
              {statistics.totalCheckins.toLocaleString()}
            </div>
          </div>
          <div className="w-12 h-12 flex items-center justify-center rounded-full bg-green-100 text-green-600">
            {" "}
            {/* [cite: 48] */}
            <i className="fas fa-check-circle"></i>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm flex items-center justify-between">
          <div>
            <div className="text-sm text-gray-500">
              Tổng địa điểm đang hoạt động {/* [cite: 49] */}
            </div>
            <div className="text-3xl font-bold text-gray-800">
              {statistics.activeCheckinPlaces.toLocaleString()}
            </div>
          </div>
          <div className="w-12 h-12 flex items-center justify-center rounded-full bg-red-100 text-red-600">
            <i className="fas fa-running"></i>
          </div>{" "}
          {/* [cite: 50] */}
        </div>
      </div>

      {/* Main Content Area */}
      <div className="p-6">
        <div className="bg-white rounded-lg shadow p-4">
          {/* Search and Action Buttons */}
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-3 sm:space-y-0 sm:space-x-4">
            <div className="flex-grow flex items-center border border-gray-300 rounded-md py-2 px-3 shadow-sm focus-within:ring-blue-500 focus-within:border-blue-500">
              {" "}
              {/* [cite: 51] */}
              <i className="fas fa-search text-gray-400 mr-2"></i>
              <input
                type="text"
                placeholder="Tìm địa điểm check-in (id & tên)"
                value={searchTerm}
                onChange={(e) => {
                  // [cite: 52]
                  setSearchTerm(e.target.value);
                  setCurrentPage(1); // [cite: 53]
                }}
                className="outline-none w-full text-gray-700"
              />
            </div>
            <div className="flex space-x-3">
              {/* Nút "Chọn xóa" / "Hủy" */}
              <button
                onClick={toggleSelectionMode} // [cite: 54]
                className={`px-4 py-2 rounded-md shadow-md transition duration-200 flex items-center
                                    ${
                                      isSelectionMode // [cite: 55]
                                        ? "bg-gray-500 hover:bg-gray-600 text-white" // [cite: 56]
                                        : "bg-orange-500 hover:bg-orange-600 text-white"
                                    }`}
              >
                <i
                  className={`fas ${
                    isSelectionMode ? "fa-times" : "fa-trash-alt" // [cite: 58]
                  } mr-2`}
                ></i>
                <span>{isSelectionMode ? "Hủy" : "Chọn xóa"}</span>{" "}
                {/* [cite: 59] */}
              </button>
              {/* Nút "Xóa đã chọn" */}
              {isSelectionMode && selectedItems.size > 0 && (
                <button
                  onClick={handleDeleteSelected}
                  className="px-4 py-2 bg-red-600 text-white rounded-md shadow-md hover:bg-red-700 transition duration-200 flex items-center" // [cite: 60]
                >
                  <i className="fas fa-trash mr-2"></i> Xóa (
                  {selectedItems.size})
                </button>
              )}{" "}
              {/* [cite: 61] */}
              {/* Nút "Thêm điểm check in" (ẩn khi đang ở chế độ chọn) */}
              {!isSelectionMode && (
                <button
                  className="px-4 py-2 bg-blue-600 text-white rounded-md shadow-md hover:bg-blue-700 transition duration-200 flex items-center"
                  onClick={() => navigate("/admin/checkin-places/create")} // [cite: 62]
                >
                  <i className="fas fa-plus mr-2"></i> Thêm điểm check in
                </button>
              )}
            </div>
          </div>{" "}
          {/* [cite: 63] */}
          {loading ? ( // [cite: 64]
            <p className="text-center text-lg text-gray-600 py-10">
              🔄 Đang tải dữ liệu, vui lòng chờ...
            </p>
          ) : filteredPlaces.length === 0 ? ( // [cite: 65]
            <p className="text-center text-lg text-gray-600 py-10">
              ⚠️ Không tìm thấy địa điểm nào phù hợp.
            </p>
          ) : (
            <>
              {/* Table */}
              <div className="overflow-x-auto">
                {" "}
                {/* [cite: 66] */}
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      {/* Checkbox "Chọn tất cả" */}
                      {isSelectionMode && ( // [cite: 67]
                        <th
                          scope="col"
                          className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        >
                          {" "}
                          {/* [cite: 68] */}
                          <input
                            type="checkbox"
                            className="form-checkbox h-4 w-4 text-blue-600 rounded" // [cite: 69]
                            onChange={handleSelectAll}
                            // Sử dụng ref để kiểm soát trạng thái checked và indeterminate
                            ref={(el) => {
                              if (el) {
                                const allFilteredSelected =
                                  selectedItems.size ===
                                    filteredPlaces.length &&
                                  filteredPlaces.length > 0;
                                const someFilteredSelected =
                                  selectedItems.size > 0 &&
                                  selectedItems.size < filteredPlaces.length;
                                el.checked = allFilteredSelected;
                                el.indeterminate = someFilteredSelected;
                              }
                            }}
                            disabled={filteredPlaces.length === 0}
                          />{" "}
                          {/* [cite: 71] */}
                          <span className="ml-2">All</span>
                        </th>
                      )}
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {" "}
                        {/* [cite: 72] */}
                        Địa điểm
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Địa chỉ {/* [cite: 73] */}
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Hạng đánh giá {/* [cite: 74] */}
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Giá(VND) {/* [cite: 75] */}
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {" "}
                        {/* [cite: 76] */}
                        Trạng thái
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Lượt check-in {/* [cite: 77] */}
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Lượt đánh giá
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        {" "}
                        {/* [cite: 78] */}
                        Hành động
                      </th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentItems.map((place) => (
                      <tr key={place.id}>
                        {" "}
                        {/* [cite: 79] */}
                        {/* Checkbox cho từng dòng */}
                        {isSelectionMode && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <input
                              type="checkbox"
                              className="form-checkbox h-4 w-4 text-blue-600 rounded" // [cite: 80]
                              checked={selectedItems.has(place.id)} // [cite: 81]
                              onChange={() => handleSelectItem(place.id)}
                            />
                          </td>
                        )}{" "}
                        {/* [cite: 82] */}
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            {renderPlaceImage(place.image, place.name)}{" "}
                            {/* [cite: 83] */}
                            <div>
                              <div className="text-sm font-medium text-gray-900">
                                {place.name} {/* [cite: 84] */}
                              </div>
                              <div className="text-xs text-gray-500">
                                ID: {place.id} {/* [cite: 85] */}
                              </div>
                            </div>
                          </div>
                        </td>{" "}
                        {/* [cite: 86] */}
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {place.address || "Đang cập nhật"} {/* [cite: 87] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {place.rating || "N/A"} {/* [cite: 88] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          {place.is_free ? ( // [cite: 89]
                            <span className="text-green-600 font-semibold">
                              Miễn phí
                            </span>
                          ) : place.price ? ( // [cite: 90]
                            `${place.price.toLocaleString()} đ`
                          ) : (
                            "Có phí"
                          )}{" "}
                          {/* [cite: 92] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {getStatusLabel(place.status)} {/* [cite: 93] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {place.checkin_count || 0} {/* [cite: 94] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {place.review_count || 0} {/* [cite: 95] */}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                          {/* Ẩn nút Sửa/Xóa đơn lẻ khi ở chế độ chọn */}
                          {!isSelectionMode && ( // [cite: 96]
                            <>
                              <button
                                className="text-blue-600 hover:underline mr-4" // [cite: 97]
                                onClick={() =>
                                  navigate(
                                    `/admin/checkin-places/edit/${place.id}` // [cite: 98]
                                  )
                                }
                              >
                                {" "}
                                {/* [cite: 99] */}
                                <i className="fas fa-edit"></i>
                              </button>
                              <button
                                className="text-red-600 hover:underline" // [cite: 100]
                                onClick={() => handleDelete(place.id)}
                              >
                                {" "}
                                {/* [cite: 101] */}
                                <i className="fas fa-times-circle"></i>
                              </button>
                            </>
                          )}{" "}
                          {/* [cite: 102] */}
                        </td>
                      </tr>
                    ))}
                  </tbody>{" "}
                  {/* [cite: 103] */}
                </table>
              </div>

              {/* Pagination Controls */}
              {totalPages > 1 && (
                <nav className="flex justify-center items-center space-x-1 mt-4">
                  <button
                    onClick={() => paginate(currentPage - 1)}
                    disabled={currentPage === 1}
                    className="px-3 py-1 text-sm text-gray-500 rounded-md hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Trước
                  </button>
                  {getPaginationNumbers().map((number, index) => (
                    <button
                      key={index}
                      onClick={() =>
                        typeof number === "number" && paginate(number)
                      }
                      className={`px-3 py-1 text-sm rounded-md ${
                        currentPage === number
                          ? "bg-blue-600 text-white"
                          : "text-gray-700 hover:bg-gray-200"
                      } ${number === "..." ? "cursor-default" : ""}`}
                      disabled={number === "..."}
                    >
                      {number}
                    </button>
                  ))}
                  <button
                    onClick={() => paginate(currentPage + 1)}
                    disabled={currentPage === totalPages}
                    className="px-3 py-1 text-sm text-gray-500 rounded-md hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Tiếp
                  </button>
                </nav>
              )}
            </>
          )}{" "}
          {/* [cite: 112] */}
        </div>
      </div>
    </div>
  );
};

export default CheckinPlaceList; // [cite: 113]
