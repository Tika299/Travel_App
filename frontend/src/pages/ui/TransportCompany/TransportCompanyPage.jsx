    ...
    const fetchData = async () => {
      try {
        ...
        let processedCompanies = filterType
          ? activeCompanies.filter(c => String(c.transportation_id) === filterType)
          : activeCompanies;
        ...
        if (filterPrice !== 'all') {
          processedCompanies = processedCompanies.filter(company => {
            ...
            if (filterPrice === 'low') return basePrice > 0 && basePrice < 10000;
            if (filterPrice === 'medium') return basePrice >= 10000 && basePrice <= 20000;
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
      ...
      <div className="relative w-full mb-10 overflow-hidden">
        <div className="relative w-full h-[360px] overflow-hidden">
          ...
          <div className="absolute inset-0 bg-black/50 flex flex-col justify-center items-start text-white text-left pl-8 md:pl-16 pr-4"> {/* <-- Thay đổi justify-center thành justify-start, items-center thành items-start */}
            <h1 className="text-4xl font-bold mb-2">
              {transportType?.name || 'Phương tiện'}
            </h1>
            <p className="text-lg mb-6">
              {transportType?.description || 'Khám phá các hãng vận chuyển phù hợp với bạn'}
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 mb-8">
        <div className="flex flex-wrap gap-4 justify-center md:justify-start">
          ...
          {/* Lọc giá */}
          <div className="relative">
            <label htmlFor="filterPrice" className="sr-only">Giá</label>
            <select
              id="filterPrice"
              className="block appearance-none w-full bg-white border border-gray-300 text-gray-700 py-2 px-4 pr-8 rounded-lg shadow-sm leading-tight focus:outline-none focus:bg-white focus:border-blue-500"
              value={filterPrice}
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {paginatedCompanies.length === 0 ? (
          <p className="text-center text-gray-600">
            ⚠️ Không có hãng vận chuyển nào đang hoạt động hoặc phù hợp với tiêu chí tìm kiếm.
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
            {paginatedCompanies.map((c) => {
              ...
              const logoDisplayUrl = c.logo
                ? (c.logo.startsWith('http') ? c.logo : ASSET_BASE_URL + c.logo)
                : (c.transportation?.icon
                  ? (c.transportation.icon.startsWith('http') ? c.transportation.icon : ASSET_BASE_URL + c.transportation.icon)
                  ...
                  <div className="p-4 flex flex-col justify-between flex-1">
                    <div>
                      ...
                      {c.tags && c.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-2">
                          {c.tags.map((tag) => (
                            ...
                            >
                              {tag.replace(/_/g, ' ')} {/* Thay thế dấu gạch dưới bằng khoảng trắng */}
                            </span>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="flex items-center mb-2">
                        {/* Icon sao */}
                        <svg className="w-4 h-4 text-yellow-500 mr-1 fill-current" viewBox="0 0 20 20">
                          <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/>
                        </svg>
                        <span className="text-sm text-gray-700">{c.rating ? c.rating.toFixed(1) : 'Chưa có'}</span>
                      </div>

                      <p className="text-sm text-gray-700 mb-1">
                        <strong>Giá khởi điểm:</strong>{' '}
                        {priceRange.base_km ? `${Number(priceRange.base_km).toLocaleString()}đ` : 'Không rõ'}
                      </p>
                      {priceRange.additional_km && (
                        <p className="text-sm text-gray-700 mb-1">
                          <strong>Giá/km:</strong> {Number(priceRange.additional_km).toLocaleString()}đ
                        </p>
                      )}
                      {c.operating_hours && (
                        <p className="text-sm text-gray-700 mb-2">
                          <strong>Thời gian hoạt động:</strong> {c.operating_hours}
                        </p>
                      )}
                      <div className="flex flex-wrap gap-2 mb-2">
                        {c.has_promotion && (
